...
Description: Contains the modified Siamese NN, which was used to predict
...
# Custom libraries
from src.data.constants import HN_MODEL_DIR
...
class SiamNet(pl.LightningModule):
    ...
    def forward(self, data):
        ...
        Note
        ----
        Images in batch input is of the form (B, V, 256, 256) where V=view
        (sagittal, transverse). Model expects image pixel intensities to be in
        [0, 255].
        """
        # Input is either a dict containing the imgs or the imgs itself
        ...
        x = x.transpose(0, 1)
        x_list = []
        for i in range(V):  # extract features for the 1+ US plane (sag, trv)
    ...
    def extract_embeddings(self, dataloader):
        """
        Extract embeddings, and return labels for all items in
        inserted dataloader.
        """
        embed_lst = []
        metadata_lst = []
        ...
        self.eval()
        for _, (imgs, metadata) in enumerate(dataloader):
            out = self.forward_embed(imgs)
            embed_lst.append(out)
            metadata_lst.append(metadata.numpy())

        embeds = np.concatenate(embed_lst, axis=0)
        metadata = np.concatenate(metadata_lst, axis=None)
...
################################################################################
def load_siamnet(dir=HN_MODEL_DIR):
    """
    Load a SiamNet model, given the folder containing the trained PyTorch
    Lightning model.

    Parameters
    ----------
    dir : str
        Directory containing hyperparameters and saved model checkpoints
    
    Returns
    -------
    SiamNet
    """
    model = SiamNet.load_from_checkpoint(f"{dir}/checkpoints/last.ckpt",
